...
def process_cases(data: pd.DataFrame):
    ...
    print(
        f"Sorted: {len(judgments)} Judgments, {len(decisions)} Decisions, {len(other)} Screening Panel"
    )
...
def sort_language(df: pd.DataFrame):
    ...
    print(
        f"Size of dataset after sorting: {len(df)} with {(df['languageisocode'] == 'ENG').sum()} English cases and {(df['languageisocode'] == 'FRE').sum()} French cases."
    )
...
def retrieve_text(data, out_dir=DATA_DIR / "case_text"):
    ...
    for _, row in data.iterrows():
        ...
        url = (
            "https://hudoc.echr.coe.int/app/conversion/docx/html/body?library=ECHR&id="
            + str(id)
        )
        try:
...
def check_retrieval(data: pd.DataFrame, case_text_dir=DATA_DIR / "case_text"):
    ...
    if os.path.exists(case_text_dir):
        downloaded_files = {
            f.replace(".html", "")
            for f in os.listdir(case_text_dir)
            if f.endswith(".html")
        }
    ...
    eclis_with_text = {
        itemid_to_ecli[itemid]
        for itemid in downloaded_files
        if itemid in itemid_to_ecli
    }
...
def data_no_dupe(data: pd.DataFrame, case_text_dir=DATA_DIR / "case_text"):
    ...
    data["case_text_path"] = (
        data["itemid"]
        .astype(str)
        .apply(lambda itemid: os.path.join(case_text_dir, f"{itemid}.html"))
    )

    # Filter to only cases where case text file exists
    data_with_text = data[data["case_text_path"].apply(os.path.isfile)].copy()
    print(
        f"Cases with retrieved text: {len(data_with_text)} (from {len(data)} original rows)"
    )
    ...
    data_sorted = data_with_text.sort_values(
        ...
        key=lambda x: (
            (x if x.name != "languageisocode" else x.map({"ENG": 0, "FRE": 1}))
            if x.name == "languageisocode"
            else x
        ),
    )
