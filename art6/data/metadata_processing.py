...
def apply_code_mapping(df, column_name, mapping):
    ...
    unmatched_codes = set(df[column_name].dropna().astype(str).unique()) - set(
        mapping.keys()
    )
    if unmatched_codes:
...
def process_judges(df, judges_csv_path, judges_output_path, unmatched_path, threshold):
    ...
    case_judge_lower_map = dict(
        zip(judges_df["Judge Name_lower"], judges_df["judge_id"])
    )
    ...
    for case_judge in unique_case_judges:
        ...
        if case_lower in case_judge_lower_map:
            ...
            best_name = judges_df[judges_df["judge_id"] == matched_id][
                "Judge Name"
            ].values[0]
            case_match_info[case_judge] = {
                "id": matched_id,
                "score": 1.0,
        ...
        for ref_lower, jid in case_judge_lower_map.items():
            ...
            if score > best_score:
                best_score = score
                best_id = jid
                best_name = judges_df[judges_df["judge_id"] == jid][
                    "Judge Name"
                ].values[0]
    ...
    df["judges_id"] = df["judges"].apply(
        lambda jl: map_list_to_ids_using_cache(jl, case_match_info, threshold)
    )
    df["judge_similarity_pct"] = df["judges"].apply(
        lambda jl: map_list_to_scores_using_cache(jl, case_match_info)
    )

    # build unmapped summary and write to a text file
    unmapped_judges = {
        j: {
            "count": case_judge_counts[j],
            "similarity": case_match_info[j]["score"],
            "best_match": case_match_info[j]["best_match"],
        }
    ...
    if unmapped_judges:
        with open(unmatched_path, "w", encoding="utf-8") as out_f:
            out_f.write(
                f"UNMAPPED JUDGES (similarity < {threshold_pct}%) - ({len(unmapped_judges)})\n"
            )
            for judge in sorted(unmapped_judges.keys()):
                info = unmapped_judges[judge]
                out_f.write(f"{judge}\n")
                out_f.write(
                    f"  Best match: {info['best_match']} ({info['similarity'] * 100:.1f}%)\n"
                )
...
def classify_art6_limb(thesaurus_labels):
    ...
    """
    if thesaurus_labels is None or (
        isinstance(thesaurus_labels, float) and pd.isna(thesaurus_labels)
    ):
        return "Unspecified"
    ...
    is_constitutional = any(
        keyword in normalized for keyword in constitutional_keywords
    )
...
def main(args):
    ...
    # enforce date type
    df["judgementdate"] = pd.to_datetime(
        df["judgementdate"], dayfirst=True, errors="raise"
    )
    ...
    df["kpthesaurus_labels"] = df["kpthesaurus"].apply(
        lambda value: map_kpthesaurus(value, key_labels)
    )
    ...
    df["law_system"] = df["country_name"].apply(
        lambda c_list: get_law_system(c_list, law_system_mapping)
    )
