        ...
        for i, df in enumerate(dfs):
            ...
            if not df.empty:
                extracted_tables.append(
                    {f"table_{i + 1}": df.to_dict(orient="records")}
                )
    except ValueError:
        pass
...
def process_corpus(
    input_dir, output_jsonl, is_decision, skip_empty_text=False, limit=None
):
