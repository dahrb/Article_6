...
WHY THIS EXISTS
---------------
This wrapper was built to install monkeypatches (response_repair,
turtle_repair, envelope_repair) ahead of the native pipeline, which is
impossible when `run_data.sh` shells out to `ontocast process` as a separate
interpreter. Those patches are gone -- ontocast v0.6.2 absorbed the JSON
bracket recovery into its own parser, and the other two never fired on any
corpus we ran -- but the wrapper is still what produces the run report below,
so it remains the entry point for every extraction arm.
...
imports that click group directly and invokes it in-process with
`standalone_mode=False` so a non-zero exit raises instead of calling
`sys.exit` out from under this wrapper. Every `ontocast process` CLI flag is
passed straight through unchanged -- this is not a reimplementation.

Usage (from the ontocast checkout, exactly like run_data.sh's own `exec`,
with PYTHONPATH pointed back at this repo so `art6.ontology` is importable):
...
ALSO WRITES a report.json next to --output-dir with wall time, per-document
unit loss (see units_lost_by_document -- the only place that signal is
recorded), and counts of unrecovered parse failures and chunk render
failures.
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Run `ontocast process` in-process, with a run report."
    )
    ...
    # Signatures matched to the two known failure classes.
    parse_failures = len(
    ...
    print(
        f"{parse_failures} unrecovered parse failure(s), "
